...
public class UserRecommendationController {
     ...
     */
    @GetMapping
    public RsData<List<UserRecommendResponse>> getRecommendations(@AuthenticationPrincipal SecurityUser loginUser) {
        Long currentUserId = (loginUser != null) ? loginUser.getId() : null;

        // 서비스에서 DTO 리스트를 직접 반환하도록 설계 변경
        List<UserRecommendResponse> recommendations = userRecommendationService.getRecommendedUsers(currentUserId);
