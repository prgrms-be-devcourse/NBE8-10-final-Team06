...
public record TechTagRes(Long id, String name, String color, String iconUrl) {

    // 1. 전체 조회/검색 API에서 Technology 엔티티를 직접 변환할 때 사용
    public static TechTagRes from(Technology technology) {
        return new TechTagRes(technology.getId(), technology.getName(), technology.getColor(), technology.getIconUrl());
    }
