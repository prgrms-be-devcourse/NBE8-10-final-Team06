...
import com.devstagram.domain.user.dto.UserRecommendResponse; // 1. 임포트 추가
...
public class UserRecommendationService {
    ...
    public List<UserRecommendResponse> getRecommendedUsers(Long currentUserId) {
        ...
        if (currentUserId == null) {
            recommendedEntities = userRepository.findAll(Sort.by(Sort.Direction.DESC, "followerCount"))
                    .stream()
                    .limit(5)
                    .toList();
        } else {
            ...
            List<Long> myTopTechIds = userTechScoreRepository.findAllByUserOrderByScoreDesc(currentUser)
                    .stream()
                    .limit(3)
        ...
        return recommendedEntities.stream()
                .map(user -> UserRecommendResponse.from(
                        user,
                        followingIds.contains(user.getId()) // 리스트에 ID가 있으면 true, 없으면 false
                ))
