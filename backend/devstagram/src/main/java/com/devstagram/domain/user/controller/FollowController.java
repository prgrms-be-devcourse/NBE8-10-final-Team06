...
public class FollowController {
    ...
    public RsData<FollowResponse> follow(@PathVariable Long toUserId, @AuthenticationPrincipal SecurityUser loginUser) {
        log.info("[FOLLOW API] POST /api/follows/{} by userId={}", toUserId, loginUser != null ? loginUser.getId() : null);
        FollowResponse response = followService.follow(loginUser.getId(), toUserId);
        log.info("[FOLLOW API] follow done: toUserId={}, isFollowing={}, followerCount={}, followingCount={}",
                response.toUserId(), response.isFollowing(), response.followerCount(), response.followingCount());
    ...
    public RsData<FollowResponse> unfollow(
        ...
        log.info("[FOLLOW API] DELETE /api/follows/{} by userId={}", toUserId, loginUser != null ? loginUser.getId() : null);
        FollowResponse response = followService.unfollow(loginUser.getId(), toUserId);
        log.info("[FOLLOW API] unfollow done: toUserId={}, isFollowing={}, followerCount={}, followingCount={}",
                response.toUserId(), response.isFollowing(), response.followerCount(), response.followingCount());
    ...
    public RsData<List<FollowUserResponse>> getFollowers(
        ...
        List<FollowUserResponse> followers = followService.getFollowers(userId, loginUserId);
